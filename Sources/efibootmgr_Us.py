...
table_commentaire.append("<  efibootmgr_Fr.py and efibootmgr_US.py are two graphical user interfaces for the efibootmgr Linux command.>")
table_commentaire.append("                ")
...
table_commentaire.append("When installing a new operating system, its installer program adds to the NVRAM a line locating its own boot ")
table_commentaire.append("module and it places the index of this new item at the top of the BootOrder table")
table_commentaire.append("The 'BootOrder' line of this screen presents the order in which the various modules are taken into account,")
table_commentaire.append("and the following lines present the different modules with their location")
table_commentaire.append("     ")
table_commentaire.append("<Warning!!!> These EFI lines are NVRAM lines and all of them may not be valid at this time .")
table_commentaire.append("there are in fact all the launchers of the EFI folders of the ESP partitions with which the computer has already")
table_commentaire.append("booted. Use Refind to see all usable launchers at boot time, and efibootmgr or the present script to reorder them.")
...
table_commentaire.append("In order to never be blocked at boot, it is in your interest, for each system, to place the Refind software ")
table_commentaire.append("launcher at the top (entry '\EFI\\refind\\refind_x64.efi') because it detects at boot time all bootable entries.")
...
table_commentaire.append("the program efibootmgr_Us.py (python GUI) also allows to change the boot order of the launchers")
...
table_commentaire.append("- Initializes the 'bootnext' variable in Nvram, which overrides the BootOrder for the next boot only  <>")
table_commentaire.append("  e.g. 'sudo --bootnext 0006' (next boot will be done by the launcher 0006), or  <>")
table_commentaire.append("  'sudo efibootmgr --delete-bootnex' to delete the 'bootnext' variable in Nvram <>")
table_commentaire.append("=================================================================================================")
...
table_commentaire.append("file name (for instance /EFI/ubuntu/shimx64.efi, or /EFI/refind/refindx64.efi)")
...
table_commentaire.append("Finally, during the installation of an OS (UBUNTU for instance) the installation program ")
table_commentaire.append("enriches the NVRAM by inscribing a row which locates the start up module(shimx64.efi for UBUNTU ")
table_commentaire.append("and grubx64.efi) and adds the rank of that new row at the top of the Boot Order table.")
...
table_commentaire.append("signature of every file xxxx.efi before launching it. However, this checking can be avoided by ")
table_commentaire.append("activating the CSM option of the BIOS (CSM: Compatibility Support Module) in order to start an OS ")
table_commentaire.append("with a non signed launcher")
table_commentaire.append("Yet, under Linux, there exists different non-signed  launchers whose launching would be refused")
table_commentaire.append("in a protected computer except by authenticate all of them.")
...
table_commentaire.append("As the bios program can only read the FAT partitions whose flag is ESP or EFI, this program")
...
table_commentaire.append("It can also create the BootNext variable which is used to override the BootOrder for only one time,")
table_commentaire.append("on the next boot.")
...
table_commentaire.append("The used commands are displayed at the bottom of each window of this GUI before being executed.")
    ...
    for efiboot in table_efiboot:  # pour chaque ligne de sudo efibootmgr -v-
        ...
        if efiboot[0:9] == "BootOrder":
            ...
            if (efiboot[0:13] == "BootCurrent: "):
                ...
                if (efiboot[0:10] == "BootNext: "):
                    global_bootNext=efiboot[10:14]
...
def faire_ou_refaire_le_boulot():
    ...
    if global_bootNext == "xxxx" :
        resultat.insert("insert", "   current boot = " + global_booCurrent + " , no 'bootnext'")
    else:
        resultat.insert("insert", "   current boot = " + global_booCurrent + "    bootnext= " + global_bootNext + " (next boot only)")
...
def popup_creat():
    ...
    label_titre2 = tk.Label(zone1, text="Label of the EFI", bg='light blue', fg='black', width=20, highlightthickness=0)
    label_titre2.place(x=205, y=35)
    label_titre3 = tk.Label(zone1, text="path of the l'EFI", bg='light blue', fg='black', width=20, highlightthickness=0)
...
#===================
    ...
    label_titre4 = tk.Label(zone5, text="Command that will be executed", bg='light blue', fg='black', width=30)
    label_titre4.place(x=275, y=90)
    ...
    def init_bootnext():
        ...
        executer["state"]="disabled"
        if global_bootNext == "xxxx" :
            label_titre0.configure(text="Current bootnext : none")
            label_titre2.configure(text="Choosing , for next boot only", bg='light blue', fg='black', width=30)
            ...
            label_titre2.configure(text="Deleting bootnext", bg='light blue', fg='black', width=30)
            combo_boots.configure(state='disabled')
            combo_boots.update()
            label_titre2.configure(text="Deleting bootnext " +global_bootNext, bg='light blue', fg='black', width=30)
    ...
    def un_boot_selecte(eventobject):
        ...
        print ("un boot selecté =" + local_bootnext)
        label_commande.configure(text=commande)
        executer["state"] = "normal"
...
def popup_ordre():
    ...
    if pos1 == "" :
        ...
        label_titre0['text'] = "'Boot current' " + global_booCurrent + " is not in 'bootorder', how is it possible ? Close all , Reboot and look at Bios"
        # =============================== bouton annuler =====================================================================
...
Bouton_creat = tk.Button(zone, width=8, text="create", command=popup_creat, relief="flat",highlightthickness=0)
Bouton_creat.config(bg='light grey', fg='black', activebackground='grey', activeforeground='black')

Bouton_modif = tk.Button(zone, width=8, text="edit", command=popup_modif, relief="flat",highlightthickness=0)
...
Bouton_nextboot = tk.Button(zone, width=8, text="Next boot", command=popup_next)
