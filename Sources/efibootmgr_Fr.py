...
#passage au répertoire contenant le programme
dir=format(os.getcwd())
os.chdir(dir)

#================== doit être root , vérifie et éventuellement, le devient===================================
euid = os.geteuid()
if euid != 0:   # vérifie que le script tourne sous super user (vrai si euid=0)
    print ("Demande privilèges Sudo.")
...
#===========================déclaration des variables qui seront globales dans les procédures
table_blkid_brute = []
table_blkid_nette = []
...
global_bootNext="xxxx"
EFI_MBR="EFI"
global bit_bouton
bit_bouton = 0


non_monté = "non"    # indicateur signalant des lignes EFI valides mais dont la partUUID n'est pas dans blkid
blancs="                                                                                 "
dir_travail = os.path.abspath(os.path.dirname(__file__))  # détermine le répertoire du programme
os.chdir(dir_travail)  # CD dossier où est "efibootmgr_Fr.py"
#================================  efibootmgr ne fonctionne bien que si sudo
if os.getuid() != 0:
    print("ce programme doit être lancé en tant que super utilisateur")
    print("Presser la touche entrée")
    sys.exit()
#--------vérifiation de la présence de blkid et efibootmgr nécessaires à ce programme----------------------
#=============   le résultat de "which efibootmgr" est "/bin/efibootmgr" ou rien si efibootmgr pas installé
try:
...
# ============================= procédure d'affichage du message si aide demandée (param "h" en fin de commande)=============================
def aidez_moi():
    ...
    if os.getuid() != 0:   #---vérification que lancé as root
        print("Comme ce n'est pas le cas, vous allez être viré dans 10 secondes.")
        time.sleep(10)
        quit()

# =================================fin de procédure aidez_moi ===================================================

# ------------------acquisition des paramètres de la ligne de commande---------------------
...
# -------------------- suppression d'anciens fichiers si nécessaire-----------------------------
...
# ======== valeurs initiales pour les fenêtres
...
bit_triEFI = 1  #----utilisé pour l'affichage des lignes EFI en ordre naturel ou selon la table BootOrder NVRAM
verbeux="0"

#===================================  composition d'un baratin explicatif =============================
#-----------------la table table_commentaire contient les lignes du baratin affichées en bas de fenêtre
...
table_commentaire.append("  ")
table_commentaire.append("Lexique : La NVRAM est une mémoire non volatile intégrée à la carte mère. Elle contient, entre autres, une liste")
table_commentaire.append("des modules exécutables lançant les différents OS (Operating System : windows, linux, ...), avec leur localisation ")
table_commentaire.append("dans les différentes unités hébergeant ces OS. Elle contient également une table de priorité indiquant dans quel")
table_commentaire.append("ordre est examinée la liste de ces modules lors du démarrage de l'ordinateur (table BootOrder ci-dessus)")
...
table_commentaire.append("localisant son propre module de démarrage, et il place ce nouveau poste en tête de BootOrder")
table_commentaire.append("La ligne 'BootOrder' de cet écran présente l'ordre de prise en compte (priorités) des différents modules, et les")
table_commentaire.append("lignes suivantes présentent les différents modules avec leur localisation")
table_commentaire.append("     ")
table_commentaire.append(" <Attention !!!> Ces lignes sont celles de la NVRAM et peuvent ne pas être toutes valides à cet instant. On y")
table_commentaire.append("trouve en effet tous les lanceurs des dossiers EFI des partitions ESP avec lesquels l'ordinateur a déjà booté.")
table_commentaire.append("Utilisez Refind pour voir tous les lanceurs au moment du boot, et efibootmgr ou le présent logiciel logiciel pour")
table_commentaire.append("les reclasser.")
table_commentaire.append("La commande 'sudo efibootmgr -v' montre les entrées de boot avec les UUID des partitions EFI les contenant, mais")
...
table_commentaire.append("de partition son nom commun linux : sda1, sdb1, sdc1.... en s'appuyant sur le résultat de la commande 'blkid'")
table_commentaire.append("Afin de ne jamais être bloqué au boot, on a intérêt, pour chaque système, à placer en tête le lanceur du logiciel")
table_commentaire.append("Refind (entrée '\EFI\\refind\\refind_x64.efi') qui, lui, détecte toutes les entrées bootables à la volée.")
table_commentaire.append("Le programme d'installation de Refind s'installe comme un lanceur dans /boot/efi/EFI/refind/refind_x64.efi. (si la")
table_commentaire.append("partition ESP est /dev/sdc1 et root est /dev/sdc2, alors /dev/sdc1/EFI/ est monté dans  /dev/sdc2/boot/efi)")
table_commentaire.append("le script efibootmgrgui_Fr permet également de changer l'ordre de démarrage des lanceurs ou d'en inscrire de ")
table_commentaire.append("nouveaux, tout comme efibootmgr")
table_commentaire.append("De même, l'installeur de Grub ajoute ses propres lanceurs /boot/efi/EFI/ubuntu/shimx64.efi et grubx64.efi ")
table_commentaire.append("Sur un systèmes dont l'EFIbios n'accepte que les lanceurs signés numériquement, on ne peut pas booter à l'aide de")
table_commentaire.append("grubx64.efi, qui n'est pas signé. Shimx64.efi par contre est signé, et est donc accepté. Il passe alors la main à")
table_commentaire.append("grubx64.efi et le tour est joué. ")
...
table_commentaire.append('  < Attention, dans ce qui suit, le \ habituel des chemins de Linux doit impérativement être remplacé par />')
...
table_commentaire.append("  (ce loader, signé, quant-à lui lance grubx64.efi, non signé, qui est dans le même répertoire). <> ")
table_commentaire.append("- Initialise en Nvram ,la variable 'bootnext' qui remplace la table bootorder lors du prochain boot seulement")
table_commentaire.append("  exemple : 'sudo --bootnext 0006' (le prochain boot sera fait avec le lanceur 0006), ou  <>")
table_commentaire.append("  'sudo efibootmgr --delete-bootnex' qui efface la variable 'bootnext' en Nvram <>")
table_commentaire.append("====================================================================================================")
table_commentaire.append("      Quelques explications concernant le démarrage des ordinateurs dits 'UEFI' ou  'EFI-BIOS'")
table_commentaire.append("Sur toute carte mère est implantée une mémoire qui ne s'efface pas après arrêt de l'ordinateur. Cette mémoire,")
table_commentaire.append("non volatile, est souvent appelée NVRAM (acronyme de 'Non Volatile Random Access Memory'). Elle contient entre")
table_commentaire.append("autres une table dont chaque poste peut indiquer le nom d'un programme dont l'exécution est immédiate avant")
table_commentaire.append("activation de toutOS (Windows, Linux,...) plus l'unité physique sur laquelle est ce programme (UUID de sa")
table_commentaire.append("partition) plus dans cette unité le chemin qui conduit à lui, plus son nom de fichier (par exemple ")
table_commentaire.append("/EFI/ubuntu/shimx64.efi).")
table_commentaire.append("Elle contient également une table indiquant dans quel ordre doivent être parcourus les postes de la table")
table_commentaire.append("précédente pour décider quel O.S lancer en premier, puis en second en cas d'échec, puis du troisième...etc. ")
table_commentaire.append("Très logiquement cette seconde table est appelée 'BootOrder', et chacun de ses postes contient le rang d'un poste ")
table_commentaire.append("de la première table, sous la forme d'un nombre de quatre chiffres en notation hexadécimale (0000,0001,...")
table_commentaire.append("...000A....) ")
table_commentaire.append("Un petit programme, aussi présent en mémoire non volatile (module du BIOS), s'exécute dès la mise sous tension.")
table_commentaire.append("Il détecte par lecture de la table BootOrder quel poste de la première table il doit exploiter et donc quel")
table_commentaire.append("programmeil doit charger pour exécution immédiate. N'étant pas très malin, il ne comprend rien d'autre que le ")
table_commentaire.append("système de fichiers FAT. Les partitions dont les UUID figurent dans la première table doivent donc être au format ")
table_commentaire.append("FAT32 afin que ce petit programme sache trouver charger et exécuter le programme de lancement de l'OS")
table_commentaire.append("Elles doivent de plus être marquées 'ESP' (ou 'EFI') et 'Bootable', sinon elles sont ignorées dans ce processus")
table_commentaire.append("")
table_commentaire.append("Sur les cartes mères de qualité, le BIOS est assez intelligent pour offrir, en affichage graphique, les outils")
table_commentaire.append("permettant de créer ou modifier des postes de la première table, et de réordonner BootOrder. Mais sur des systèmes")
table_commentaire.append("rustiques ou anciens, cet outil est très rudimentaire ou absent. Dans ce dernier cas, les tables ne peuvent être")
table_commentaire.append("modifiées que par un programme, chargé en cas de problème depuis un CD ou une clé USB bootable.")
...
table_commentaire.append("enrichit la NVRAM en y inscrivant une ligne localisant le module de démarrage (shimx64.efi pour UBUNTU), et il")
table_commentaire.append("ajoute le rang de ce nouveau poste en tête de BootOrder ")
table_commentaire.append("Au démarrage suivant, c'est donc dans cet exemple shimx64.efi qui sera exécuté, lequel lancera UBUNTU.  ")
table_commentaire.append("")
table_commentaire.append("Remarque : un mécanisme d'authentification a été imaginé pour protéger l'ordinateur contre l'installation de")
table_commentaire.append("programmes de démarrage non authentiques. Ce mécanisme est inclus dans le BIOS, qui vérifie la 'signature' de")
table_commentaire.append("chaque programme avant de le lancer.  ")
table_commentaire.append("Or, différents lanceurs existent sous Linux, non signés, pour lesquels dans un ordinateur protégé le lancement ")
table_commentaire.append("serait refusé sauf à  tous les authentifier préalablement, ce qui a un coût.")
table_commentaire.append("C'est pourquoi on utilise le programme 'shimx64.efi' qui, lui, est signé, et dont le seul rôle est de lancer le")
table_commentaire.append("programme grubx64.efi (c'est le boot loader de Grub) propre à chaque variante de Linux.")
table_commentaire.append("")
table_commentaire.append("========================================= but du présent programme ===============================")
table_commentaire.append("Le BIOS ne sachant lire que les partitions FAT32  dont le drapeau est 'ESP' (ou EFI), Le présent programme se")
table_commentaire.append("limite également à ne présenter que ces unités sous la forme sda1, sdb1, sdc2....selon la convention de Linux.")
table_commentaire.append("Il permet de créer, modifier, supprimer des postes de la table listant les programmes de démarrage des OS, et ")
table_commentaire.append("de modifier le classement de la table BootOrder.")
table_commentaire.append("Il peut aussi initialiser la variable 'bootnext qui est utilisée pour remplacer la table 'bootorder' pour une")
table_commentaire.append("fois seulement, lors du prochain boot (ce qui permet de tester un nouveau lanceur sans risque)")
table_commentaire.append("   ")
table_commentaire.append("En fait, il compose en fonction des indications qui lui sont fournies par l'opérateur et du contenu de la NVRAM")
table_commentaire.append("les commandes que l'on aurait pu passer laborieusement à la main dans un terminal pour arriver au même résultat.")
table_commentaire.append("Les commandes utilisées sont affichées en bas de fenêtre avant d'être exécutées.")
...
def extrac_dev_partUUID_de_blkid():
    ...
    for blkid in table_blkid_brute:
        ...
        #--  Au final, pour trouver le device associé à chaque poste de efibootmgr, il suffit de conserver device + partUUID dans table_blkid_nette
...
def extract_efibootmgr():
    ...
    BootOrder = "pas de Boot0rder, EFIbios agira par défaut ou faites ""sudo efibootmgr -o 0000,0001,0002 par exemple"""   # initialisation bootorder
...
def croisement_efibootmgr_blkid_selon_UUID():
    ...
    non_monté = "non"
    for j in range(0, len(table_efibootmgr_net)):   #---- pour chaque ligne EFI de efibootmgr, cherche parmi toutes blkid la même UUID
        ...
        for i in range(0, len(table_blkid_nette)):
            ...
            if table_efibootmgr_net[j][34:70] == table_blkid_nette[i][13:49]:  # dans table_efi_nette, partUUID est de 13 à 49
                ...
                break       #------- si trouvé, arrêter la recherche d'égalité, sdx trouvé, casser la boucle
            # !!!!!!!!!!!! attention, il se peut qu'on ne trouve pas d'unité (démontée) et alors table_EFIsdx[rang_part] est resté à "...???..."

        if (table_EFIsdx[len(table_EFIsdx)-1] =="...??..."):
            non_monté = "oui"

        #============ fin de procédure de croisement des tables  ================================================================

#=======================================procédure d'affichage dans window et dans terminal=================
def affichage():
    ...
    table_EFI_Entete = []   #-------------------- quatre lignes d'entête dans  table_EFI_Entete
    table_EFI_Entete.append("                    ")
    table_EFI_Entete.append("                                              table efibootmgr  ")
    table_EFI_Entete.append("   " + BootOrder[0:len(BootOrder) - 1])  # retire le saut de ligne à droite de la chaîne BootOrder
    table_EFI_Entete.append("     rang       libellé                      Lanceur dans ESP                     UUID de la partition  ")
    ...
    if (postes_efi_en_trop == "oui"):
        table_EFI_print.append('   "?" à gauche d''un poste signale que ce poste n''est pas cité dans BootOrder')
...
def recadrer():
    ...
    place_bouton=  (largeur_fenetre - retirer - Bouton_quitter.winfo_width() - Bouton_actualise.winfo_width())/7
    ...
    x1 =place_bouton*4
    Bouton_nextboot.place(x=x1, y=0)
...
def faire_ou_refaire_le_boulot():
    ...
    if global_bootNext == "xxxx":
        resultat.insert("insert", "   Lanceur actuel = " + global_booCurrent + " , pas de 'bootnext'")
    else:
        resultat.insert("insert", "   Lanceur actuel = " + global_booCurrent + "    bootnext= " + global_bootNext + " (prochain boot uniquement)")
...
def popup_creat():
    ...
    def executer_commande():
        ...
        global path_EFI
        if volume=="":
            label_titre0['text'] ="Vous avez oublié de choisir la partition"
            return
        ...
        if (path_EFI == "/EFI/") or (path_EFI == ""):
            label_titre0['text'] = "Vous avez oublié le libellé de l'EFI"
            return
        commande = "sudo efibootmgr --create --disk " + volume + " --part " + partition + " --label " + '"' + libellé_EFI.rstrip(" ") + '"' + " --loader " + path_EFI.rstrip(" ")
    ...
    label_titre2 = tk.Label(zone1, text="Libellé de l'EFI", bg='light blue', fg='black', width=20, highlightthickness=0)
    ...
    label_titre4 = tk.Label(zone1, text="Commande qui sera exécutée", bg='light blue', fg='black', width=30)
...
# ici
    ...
    executer.config(text="Exécuter & quitter")
    executer.pack(padx=2, pady=8, side="top")
    executer.place(x=1, y=1)
    executer.config(height=1, width=12)
    # ================================= fin bouton executer et quitter ================================
    ...
    label_titre2 = tk.Label(zone5, text="Choix de l'élément à modifier", bg='light blue', fg='black', width=30)
    ...
    def change_libel_chemin(libellé_EFI,chemin):
        ...
        commande2=""
        if len(libellé_EFI) > 20 :
            label_anomalie.configure(text="La longueur du libellé du poste EFI est limitée à 20 caractères")
            value1.set(libellé_EFI[:20])
            return
        ...
        # if len(chemin) > 30 :
        if len(chemin) > 40:
            label_anomalie.configure(text="La longueur du chemin du poste EFI est limitée à 40 caractères")
...
#=================================================================================================================================
    label_titre4 = tk.Label(zone5, text="Commandes qui seront exécutées", bg='light blue', fg='black', width=30)
    label_titre4.place(x=275, y=110)

    label_commande = tk.Label(zone5, text="commande qui sera executée", bg='ivory', fg='black', width=111)
    label_commande.place(x=10, y=135)

    label_commande2 = tk.Label(zone5, text="commande qui sera executée", bg='ivory', fg='black', width=111)
    label_commande2.place(x=10, y=155)

    label_commande3 = tk.Label(zone5, text="commande qui sera executée", bg='ivory', fg='black', width=111)
...
def popup_suppr():
    ...
    for i in range (0,len(table_lignes_affichees)):
        if bb in table_lignes_affichees[i]:
            pass    #le bootcurrent n'est pas présenté pour suppression
        else :
    ...
    # table_sauf_bootcurrent ne présentera donc pas le bootcurrent pour suppression
    combo_boot.place(x=10,y=65)
    combo_boot.current(index_combo)
    combo_boot.bind("<<ComboboxSelected>>", un_boot_selecte)  # ==action "un_boot_selecte" quand uévénement ComboboxSelected
    # ============================ fin liste partitions possibles ============================================

    label_titre0 = tk.Label(zone2, text=" ", bg='light blue', fg='red', width=66)

    label_titre0.place(x=135, y=0)
    label_titre4 = tk.Label(zone2, text="Commande qui sera exécutée", bg='light blue', fg='black', width=30)
    label_titre4.place(x=275, y=90)
    label_titre2 = tk.Label(zone2, text="Choix de l'élément à supprimer", bg='light blue', fg='black', width=30)
    label_titre2.place(x=275, y=35)

    label_commande = tk.Label(zone2, text="commande qui sera executée", bg='ivory', fg='black', width=97)
    label_commande.place(x=10, y=115)
    #=========initialisation===============
    boot_choisi = combo_boot.get()
    Boot_a_suppr = combo_boot.get()[3:12]
...
def popup_next():
    ...
    global table_lignes_affichees
    global commande
    global global_bootNext
    global local_bootnext
    commande=""
    largeur_wincourante=800
    hauteur_wincourante=150
    win_next = tk.Toplevel()		  # Popup -> Toplevel()
    win_next.transient(window)
    win_next.title('BootNext')

    zone5 = tk.Frame(win_next, bg='light blue')
    zone5.pack(padx=1, pady=1, fill='both', expand=1)
    zone5.update()

    label_titre0 = tk.Label(zone5, bg='light blue', fg='black', width=66)
    label_titre0.place(x=135, y=0)
    ...
    #combo_boots = ttk.Combobox(zone5, width=95, values=table_bootnum)
    combo_boots = ttk.Combobox(zone5, width=95, values=table_lignes_affichees)
    combo_boots.place(x=10, y=65)
    combo_boots.current(index_combo)

#===================
    # calcul position x, y
    x = coin_x + (int(largeur_fenetre)/2)  - (int(largeur_wincourante) / 2)
    y = coin_y + (int(hauteur_fenetre)/2) -(int(hauteur_wincourante) / 2) + 100
    win_next.geometry('%dx%d+%d+%d' % (largeur_wincourante, hauteur_wincourante, x, y))
    win_next.config(bg="cyan")

    label_commande = tk.Label(zone5, text="sudo efibootmgr ....", bg='ivory', fg='black', width=97)
    label_commande.place(x=10, y=115)

    label_titre4 = tk.Label(zone5, text="Commande qui sera exécutée", bg='light blue', fg='black', width=30)
    label_titre4.place(x=275, y=90)
    label_titre2 = tk.Label(zone5, bg='light blue', fg='black', width=30)



    local_bootnext="xxxx"
    # ================================ bouton executer et quitter ===================================
    executer = tk.Button(zone5, activebackground = "red", bg = "cyan", fg = "black",  width = 9,highlightthickness=0)
    executer.config(text="Run & Exit")
    executer.pack(padx=2, pady=8, side="top")
    executer.place(x=1, y=1)
    executer.config(height=1, width=12)
    # ================================= fin bouton executer et quitter ================================
    executer['state'] = "disabled"

    def init_bootnext():
        global commande
        executer.config(command=executer_bootnext, )
        executer["state"]="disabled"
        if global_bootNext == "xxxx" :
            label_titre0.configure(text="Variable bootnext : vide")
            label_titre2.place(x=200, y=35)
            label_titre2.configure(text="Choisir un lanceur, pour le prochain boot seulement", bg='light blue', fg='black', width=50)
            combo_boots.configure(state='normal')
            combo_boots['state'] = 'normal'
            combo_boots.configure(value=table_lignes_affichees)
            combo_boots.update()
            local_bootnext = global_bootNext
            label_commande.configure(text="sudo efibootmgr --bootnext xxxx")
            commande="sudo efibootmgr --bootnext xxxx"
        else :
            label_titre0.configure(text="  bootnext " + global_bootNext)
            label_titre2.configure(text="Effacer bootnext", bg='light blue', fg='black', width=30)
            combo_boots.configure(state='disabled')
            combo_boots.update()
            label_titre2.place(x=275, y=35)
            label_titre2.configure(text="Effacer bootnext " +global_bootNext, bg='light blue', fg='black', width=30)
            label_commande.configure(text="sudo efibootmgr --delete-bootnext")
            commande="sudo efibootmgr --delete-bootnext"
            local_bootnext = "xxxx"
            executer["state"] = "normal"
    ...
    def un_boot_selecte(eventobject):
        ...
        global local_bootnext
        global table_lignes_affichees
        local_bootnext = combo_boots.get()[7:12]
        commande = "sudo efibootmgr -n " + local_bootnext
        label_commande.configure(text=commande)
        executer["state"] = "normal"

    combo_boots.bind("<<ComboboxSelected>>", un_boot_selecte)

    def annuler_bootnext():
        win_next.destroy()

    def executer_bootnext():
        global commande
        global global_bootNext
        if "sudo efibootmgr -n" in commande:    # créer un bootnext
            global_bootNext = local_bootnext
        else:
            global_bootNext = "xxxx"
        code_retour = os.system(commande)
        if code_retour != 0:
            print("crash " + commande)

        faire_ou_refaire_le_boulot()
        win_next.destroy()

    combo_boots.bind("<<ComboboxSelected>>", un_boot_selecte)  # ==action "un_boot_selecte" quand uévénement ComboboxSelected


    annuler = tk.Button(zone5, activebackground = "red", bg = "cyan", fg = "black", command = annuler_bootnext, width = 9,highlightthickness=0)
    annuler.config(text="Cancel")
    annuler.pack(padx=2, pady=8, side="top")
    annuler.place(x=674, y=1)
    annuler.config(height=1, width=12)
    ...
    init_bootnext()
    win_next.transient(window) 	  # Réduction popup impossible
    win_next.grab_set()		  # Interaction avec fenetre window impossible
    window.wait_window(win_next)   # Arrêt script principal
...
def popup_ordre():
    ...
    Boot_Actuel.configure(tabs=('8.45c', 'center'))  # tabulation centrée à 8 cm du bord gauche
    ...
    label_choix_poste= tk.Label(zone3, text="choisir un poste à déplacer", bg='light blue', fg='black', width=27)
    label_choix_poste.place (x=200, y=95)

    label_depl = tk.Label(zone3, text="--déplace--", bg='light blue', fg='grey', width=9)
    ...
    def executer_commande():
        ...
        l = len(un) - 1  # un saut de ligne à droite, à enlever
        un = un[0:l]
...
def popup_quitter():
    print ("et là, presser la touche entrée !!!")
    sys.exit()

Bouton_creat = tk.Button(zone, width=8, text="créer", command=popup_creat, relief="flat",highlightthickness=0)
...
Bouton_nextboot = tk.Button(zone, width=8, text="Prochain Boot", command=popup_next)
Bouton_nextboot.config(bg='light grey', fg='black', activebackground='grey', activeforeground='black', relief="flat",highlightthickness=0)
...
chk_triEFI: Checkbutton = tk.Checkbutton(zone, width=16, text="Trié / BootOrder", bd=4 , var=chk_triEFI, command=chk_triEFI)
