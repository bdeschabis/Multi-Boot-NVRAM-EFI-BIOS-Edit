...
table_commentaire.append("   ")
table_commentaire.append("Lexique : la NVRAM est une mémoire non volatile conservée sur la carte mère. Elle contient, entre autres, une liste")
...
table_commentaire.append("     ")
table_commentaire.append(" <Attention !!!> Ces lignes sont celles de la NVRAM et peuvent ne pas être toutes valides à cet instant.")
table_commentaire.append("On y trouve en effet tous les lanceurs des dossiers EFI des partitions ESP avec lesquels l'ordinateur a déjà booté.")
...
table_commentaire.append("(si la partition ESP est /dev/sdc1 et root est /dev/sdc2, alors /dev/sdc1/EFI/  est monté dans  /dev/sdc2/boot/efi)")
table_commentaire.append("le script efibootmgrgui_Fr permet également de changer l'ordre de démarrage des lanceurs ou d'en inscrire de nouveaux,")
...
table_commentaire.append("Sur un systèmes dont l'EFIbios n'accepte que les lanceurs signés numériquement, on ne peut pas démarrer à l'aide de")
...
table_commentaire.append("Sur toute carte mère est implantée une mémoire qui ne s'efface pas même après arrêt de l'ordinateur. Cette mémoire,")
table_commentaire.append("non volatile, est souvent appelée NVRAM (acronyme de 'Non Volatile Random Access Memory'). Elle contient entre autres")
table_commentaire.append("une table dont chaque poste peut indiquer le nom d'un programme dont l'exécution est immédiate avant activation de tout")
table_commentaire.append("OS (Windows, Linux,...) plus l'unité physique sur laquelle est ce programme (UUID de sa partition) plus dans cette")
table_commentaire.append("unité le chemin qui conduit à lui, plus son nom de fichier (par exemple /EFI/ubuntu/shimx64.efi).")
table_commentaire.append("Elle contient également une table indiquant dans quel ordre doivent être parcourus les postes de la table précédente")
table_commentaire.append("pour décider quel système lancer en premier, puis en second en cas d'échec du premier, puis du troisième...etc. ")
...
table_commentaire.append("de la première table, sous la forme d'un nombre de quatre chiffres en notation hexadécimale (0000,0001,...000A....) ")
table_commentaire.append("Un petit programme, lui aussi présent en mémoire non volatile (on l'appelle BIOS) s'exécute dès la mise sous tension.")
table_commentaire.append("Il détecte par lecture de la table BootOrder quel poste de la première table il doit exploiter et donc quel programme")
table_commentaire.append("il doit charger pour exécution immédiate. N'étant pas très intelligent, il ne comprend rien d'autre que le système de")
table_commentaire.append("fichiers FAT32. Les partitions dont les UUID figurent dans la première table doivent donc être au format FAT32 afin")
table_commentaire.append("que ce petit programme sache trouver charger et exécuter le programme de lancement de l'OS")
...
table_commentaire.append("enrichit la NVRAM en y inscrivant une ligne localisant le module de démarrage (shimx64.efi pour UBUNTU), et il ajoute")
table_commentaire.append("le rang de ce nouveau poste en tête de BootOrder ")
...
table_commentaire.append("")
table_commentaire.append("Remarque : un mécanisme d'authentification a été imaginé pour protéger l'ordinateur contre l'installation de programmes")
table_commentaire.append("de démarrage non authentiques. Ce mécanisme est inclus dans le BIOS, qui vérifie la 'signature' de chaque programme")
table_commentaire.append("avant de le lancer.  ")
table_commentaire.append("Or, différents lanceurs existent sous Linux, non signés, pour lesquels dans un ordinateur protégé le lancement serait")
table_commentaire.append("refusé sauf à  tous les authentifier préalablement.")
...
table_commentaire.append("Le BIOS ne sachant lire que les partitions FAT32  dont le drapeau est 'ESP' (ou EFI), Le présent programme se limite")
table_commentaire.append("également à ne présenter que ces unités sous la forme sda1, sdb1, sdc2....selon la convention de Linux.")
...
table_commentaire.append("il permet également de modifier le classement de la table BootOrder.")
table_commentaire.append("")
...
table_commentaire.append("que l'on aurait pu passer laborieusement à la main dans un terminal pour arriver au même résultat.")
